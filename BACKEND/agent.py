    ...
    try:
        ...
        for attempt in range(1, 11):
            async with AsyncSessionLocal() as db:
                ...
                if call:
                    print(f"[CALL LOOKUP] call_id={call_id} attempt={attempt} found=true")
                    break
                else:
                    print(f"[CALL LOOKUP] call_id={call_id} attempt={attempt} found=false")
            await asyncio.sleep(1)

        if not call:
            print(f"[CALL LOOKUP FAILURE] call_id={call_id} reason=call_row_missing_after_dispatch")
            return None
        ...
        print(f"[agent] Exception in _get_campaign_info for call {call_id}: {e}")
        return None
...
async def entrypoint(ctx: JobContext):
    ...
    def on_track_subscribed(track: rtc.Track, publication: rtc.TrackPublication, participant: rtc.RemoteParticipant):
        if track.kind == rtc.TrackKind.KIND_AUDIO and participant.identity == "customer":
            _safe_create_task(record_track(track, call_id, speaker="customer", answered_event=call_answered_event, disconnected_event=customer_disconnected_event), name="record_track_customer", call_id=call_id)
    ...
    try:
        ...
        if not customer_answered:
            ...
            greeting_instructions = (
                ...
                "Deliver the opening greeting, ask the first question, and wait for the customer's response. "
                "Do not invoke finish_call during the initial greeting. "
                ...
                "You are now starting the call. Deliver the opening greeting, ask the first question, and wait for the customer's response. "
                "Do not invoke finish_call during the initial greeting. "
                "Begin EXACTLY at STEP 1 of the campaign script — "
