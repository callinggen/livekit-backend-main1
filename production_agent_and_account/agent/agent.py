...
def mix_wav_files(file1: str, file2: str, output_file: str):
        ...
        for f in (file1, file2):
            try:
                if os.path.exists(f):
                    ...
                    # NOTE: Don't delete here — cleanup_track_files() handles this after S3 verify
                    return
    ...
    try:
        ...
        # NOTE: Individual track files are NOT deleted here.
        # They will be deleted by cleanup_track_files() in s3_service.py
        # ONLY after the mixed file is confirmed uploaded to S3.
    except Exception as e:
...
async def entrypoint(ctx: JobContext):
    ...
    try:
        ...
        # Guard against duplicate agent workers joining the same room
        other_agents = [
            p for p in ctx.room.remote_participants.values()
            if p.identity != ctx.room.local_participant.identity and (p.identity.startswith("agent-") or "callinggen-agent" in p.identity.lower() or "agent" in p.identity.lower())
        ]
        if other_agents:
            print(f"[agent] Duplicate agent worker detected in room {ctx.room.name}. Exiting extra worker.")
            return
        ...
        async def _handle_room_disconnect():
            ...
            # Mix WAV tracks & upload to S3 (deleting local file immediately)
            rec_url = None
            if call_id != -1:
                try:
                    ...
                    from app.services.s3_service import upload_to_s3_and_delete_local
                    rec_url = upload_to_s3_and_delete_local(local_wav)
            ...
            try:
                await notify_call_complete(
                    ...
                    payload={
                        ...
                        "recording_url": rec_url or (f"/api/recordings/call_{call_id}.wav" if call_id != -1 else None),
                    },
                )
            except Exception as e:
...
if __name__ == "__main__":
    ...
    agent_name = os.getenv("LIVEKIT_AGENT_NAME", "callinggen-agent-prod")
